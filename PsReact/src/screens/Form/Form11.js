...
export default function Form3 ({navigation}){
    ...
    const getPergunta = async () => {
        const response = await api.get('http://127.0.0.1:8000/api/v1/Pergunta/11/');
    ;
    ...
    return( 
        ...
        feedback ==="") ? ( 
            ...
            <TouchableOpacity style={styles.buttom} 
            onPress={() => (
                data(importance, satisfaction, feedback),navigation.navigate('Form12'))}>
                <Icon name="chevron-small-right" style={styles.icon}></Icon>
             </TouchableOpacity>
        )}
            </View>  
